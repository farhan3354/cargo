import { getVideos } from "@/app/actions/admin";
import { VideoForm } from "./VideoForm";
import VideosList from "./VideosList";

export default async function VideosPage() {
  let videos: Awaited<ReturnType<typeof getVideos>> = [];
  try {
    videos = await getVideos();
  } catch {
    videos = [];
  }

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-[#110713]">Manage Videos</h1>
        <p className="text-[#66556B] mt-2">Add and manage promotional or instructional videos.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white p-6 rounded-2xl shadow-sm border border-[#E5E7EB]">
          <h2 className="text-xl font-semibold mb-4 text-[#110713]">Add New Video</h2>
          <VideoForm />
        </div>

        <div className="lg:col-span-2">
          <h2 className="text-xl font-semibold mb-4 text-[#110713]">Current Videos</h2>
          <div className="bg-white rounded-2xl shadow-sm border border-[#E5E7EB] overflow-hidden">
            <VideosList initialVideos={videos as any} />
          </div>
        </div>
      </div>
    </div>
  );
}
